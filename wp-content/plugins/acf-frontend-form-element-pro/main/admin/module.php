...
if( ! class_exists( 'Frontend_WP_Admin' ) ) :
	...
	class Frontend_WP_Admin{
		...
		public function activate_payments_addon(){
			...
			wp_redirect( add_query_arg( array( 'page' => FEA_PRE.'-settings', 'tab' => 'payments' ), admin_url() ) );
		...
		public function settings_sections(){
			...
			foreach( $this->tabs as $tab => $label ){
				if( ! in_array( $tab, [ 'welcome', 'payments', 'acf' ] ) ) require_once( __DIR__ . "/admin-pages/main/$tab.php" );
			}
		...
		public function scripts(){
			...
			if ( fea_instance()->is__premium_only() ) {
				wp_register_style( 'fea-public-pro', FEA_URL . 'assets/css/pro/frontend-admin-pro'.$min.'.css', array(), FEA_VERSION );
				wp_register_script( 'fea-public-pro', FEA_URL . 'assets/js/pro/frontend-admin-pro'.$min.'.js', array( 'fea-public' ), FEA_VERSION, true );
			}
