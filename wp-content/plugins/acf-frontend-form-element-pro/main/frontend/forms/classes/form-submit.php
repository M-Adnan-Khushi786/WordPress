	...
	class Submit_Form {
		...
		function validate_values( $values, $input_prefix = '' ) {
			...
			foreach ( $values as $key => $value ) {
				if( ! in_array( $key, $current_keys ) ) continue;
					...
					if ( $key == 'acff' ) {
						...
						foreach ( $value as $k => $inputs ) {
							if( in_array( $k, $actions ) ){
								...
								$record = $this->add_value_to_record( $record, $k, $inputs, false, $objects );
							}
						}		
					}else{
		...
		public function add_value_to_record( $record, $key, $input, $group = false, $objects = false ){
		...
		public function return_form( $form ){
			...
			if ( fea_instance()->is__premium_only() ){
				...
				if( isset( $form['record']['_acf_step'] ) ){
					$form['step_index'] = $form['record']['_acf_step'];
					$step = $form['steps'][ $form['step_index'] ];
					if( $form['step_index'] < count( $form['steps'] ) ){
						$this->reload_form( 0, $form, $step );
					}				
				}
		...
		public function reload_form( $form_ids, $form, $step ){
			...
			wp_send_json_success( [ 'reload_form' => $reload_form, 'step' => $form['step_index'] ] );
